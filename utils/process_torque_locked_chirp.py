...
def process(yaml_file, plot_all=False):
    ...
    image_base_path = new_head +f'{code_string}_test-torque-locked-chirp'

    log_file = head + f'{code_string}_test-torque-locked-chirp.log'
    print('[i] Reading log_file: ' + log_file)

    # log format: '%u64\t%f\t%f\t%f\t%f'
    ns        = [np.uint64(x.split('\t')[0]) for x in open(log_file).readlines()]
    ref_tor   = [    float(x.split('\t')[1]) for x in open(log_file).readlines()]
    motor_tor = [    float(x.split('\t')[2]) for x in open(log_file).readlines()]
    motor_vel = [    float(x.split('\t')[3])/1000 for x in open(log_file).readlines()]
    i_fb      = [    float(x.split('\t')[4]) for x in open(log_file).readlines()]

    if 'test_torque_locked_chirp' in out_dict:
        A = out_dict['test_torque_locked_chirp']['A']
        duration = out_dict['test_torque_locked_chirp']['duration']
        freq_0 = out_dict['test_torque_locked_chirp']['freq_0']
        freq_f = out_dict['test_torque_locked_chirp']['freq_f']
    else:
        raise Exception("missing 'test_torque_locked_steps' in yaml parsing")
    ...
    plt_max = (max(ref_tor[0:10000]) -min(ref_tor[0:10000])) * 0.05
    axs[0].set_ylim(min(ref_tor[0:10000])-plt_max,max(ref_tor[0:10000])+plt_max)
    ...
    w, mag, phase = bode_utils.bode([s/1e9 for s in ns],ref_tor,motor_tor)
    id_0= (np.abs(w - 1) <= 0.01).argmax()
    id_1= (np.abs(w - freq_f) <= 0.01).argmax()
    w_f = w[id_0:id_1+1]
