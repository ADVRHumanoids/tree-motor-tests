...
def process(yaml_file, plot_all=False):
    ...
    image_base_path = new_head +f'{code_string}_test-current-locked-steps'

    log_file = head + f'{code_string}_test-current-locked-steps.log'
    ...
    if 'test_current_locked_steps' in out_dict:
        c_steps = out_dict['test_current_locked_steps']['target_cur']
        t_steps = [ns[0]/1e9+t for t in out_dict['test_current_locked_steps']['time']]
    else:
    ...
    # motor_vel vs i_q ------------------------------------------------------
    fig, axs = plt.subplots()
    axs.plot(i_fb,  motor_tor, color='#8e8e8e', marker='.', markersize=0.2, linestyle="", label='current out fb (A)')
    step_ids=[id for id in range(1,len(i_q)) if i_q[id]!=i_q[id-1]]
    for i in range(1,len(step_ids)):
        axs.plot(i_q[step_ids[i-1]:step_ids[i]], motor_tor[step_ids[i-1]:step_ids[i]], marker='.', markersize=0.5, linestyle="", label=f'ref: {i_q[step_ids[i-1]]:5.2f} A')
    lgnd = axs.legend()
    for handle in lgnd.legendHandles:
        handle._legmarker.set_markersize(6)
